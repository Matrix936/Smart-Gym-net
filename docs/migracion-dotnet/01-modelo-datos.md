# 01 — Modelo de Datos (copia literal del schema)

**Fuente:** `docs/schema_smart_gym.sql` — Schema SQLite v2 (FK reales). Copia literal, sin ediciones.

**Regla de rescate:** el schema se porta **tal cual**. No rediseñar tablas ni columnas durante la migración. Cualquier cambio debe ser una decisión explícita documentada en esta carpeta, no un ajuste incidental al traducir código.

## Hechos verificados (conteo sobre el archivo)

- **30 tablas** (`CREATE TABLE IF NOT EXISTS`)
- **20 triggers** de actualización automática de `updated_at` (uno por tabla sincronizable)
- **30 índices** (`CREATE INDEX IF NOT EXISTS`)
- **1 seed mínimo** (rol `SUPERADMIN` + sede `Sede Principal`)

## Convenciones vigentes (de los comentarios del propio schema)

1. `snake_case` en tablas y columnas.
2. Prefijo `id_` en columnas FK; la FK declarada apunta exactamente a la PK referenciada.
3. Dinero SIEMPRE `INTEGER`, sufijo `_centavos`. Nunca `REAL`/`FLOAT`.
4. IDs: `INTEGER AUTOINCREMENT` para catálogos; `TEXT` (UUID v4) para transaccionales.
5. Sin `ON DELETE CASCADE` por defecto (comportamiento NO ACTION/RESTRICT de SQLite) — casi todo usa soft delete (`deleted_at`).
6. Tablas creadas en orden de dependencia (toda tabla referenciada se crea antes que la que la referencia).
7. **4 tablas NO sincronizables** (sin columnas de sync): `socios_biometricos`, `perifericos_config`, `sesiones`, `cuentas_recordadas_local`.
8. `PRAGMA foreign_keys = ON` requerido en **cada** conexión.

## Clasificación de tablas

| Grupo | Tablas |
|---|---|
| Seguridad y configuración | `roles`, `permisos_rol`, `sedes`, `usuarios`, `sesiones`, `perifericos_config`, `cuentas_recordadas_local`, `configuracion_general`, `empresa_config_fiscal` |
| Socios | `socios`, `socios_biometricos`, `socios_historial_estado` |
| Catálogos independientes | `planes_membresia`, `dispositivos_acceso`, `categorias_productos`, `productos`, `inventario_sucursal` |
| Caja | `cajas_sesiones`, `caja_movimientos` |
| Membresías | `membresias`, `membresias_pagos`, `membresias_congelamientos` |
| Control de acceso | `accesos_bitacora` |
| POS / Ventas | `ventas`, `detalle_ventas` |
| Cobranza | `cuentas_cobrar`, `cobros_cuotas`, `cobros_recordatorios` |
| Auditoría | `bitacora_auditoria` |
| Migraciones | `schema_migrations` (backfills one-shot) |

## Referencias polimórficas (NO son FK reales)

- `caja_movimientos.referencia_tipo` + `referencia_id` → apunta a `ventas.id_venta` o `membresias_pagos.id_pago`.
- `bitacora_auditoria.id_registro_afectado` → referencia polimórfica por `tabla_afectada`.
...
## Notas para la implementación en .NET

- **Triggers y FKs se ejecutan tal cual en SQLite** — en .NET usar Microsoft.Data.Sqlite (no EF Core que ejecuta SQL por sí mismo; los triggers son parte del schema, no del ORM).
- Si se usa EF Core para consultas, el schema de la DB lo sigue mandando este archivo (migraciones "empty" o ejecución directa del script en el arranque), NO `EnsureCreated` que generaría un schema distinto.
- `PRAGMA foreign_keys = ON` debe ejecutarse en cada conexión (como ya hace `db.rs` / `create_sqlite_connection`).
- Las fechas son TEXT ISO8601 UTC (`strftime('%Y-%m-%dT%H:%M:%fZ','now')`) — mantener el mismo formato para compatibilidad con sync.
